"""
Mode A: Direct Optical (RGB) Pipeline.
RGB -> YOLOv5su -> Detections
"""
...
class RGBPipeline(BasePipeline):
    def __init__(self, checkpoint: str = "checkpoints/yolov5su_rgb_best.pt"):
        super().__init__("RGB — YOLOv5su")
    ...
    def run(
        ...
        **kwargs
        ...
        if rgb_img is None:
            raise ValueError("RGB Pipeline requires a valid RGB image input.")

        timer = PipelineTimer()

        # 1. Preprocessing stage
        timer.start_stage("preprocess_ms")
        proc_img = rgb_img.copy()
        timer.end_stage()

        # 2. Detector inference stage
        timer.start_stage("detector_ms")
        detector = self.manager.get_yolo_detector("yolov5su_rgb", self.checkpoint)
